...
HDRN = HDRN + (rand(size(HDRN))-0.5)./20;
TimeHDRN = 0:1:size(HDRN,2)-1;
TimeHDRN = repmat(TimeHDRN,size(HDRN,1),1);
TimeHDRN = TimeHDRN + rand(size(TimeHDRN))./5;

scatter(TimeHDRN,HDRN,'filled', 'MarkerFaceColor', ColorID,...
        'SizeData', 1);
p6 = plot(TimeHDRN(:,:)',HDRN(:,:)','Marker','none','LineStyle','-','LineWidth',0.5,'Color',[cmaplines(1,:) 0.2]);
...
ylim([-0.25 1.2]);
plot([-1 10], [0 0], 'LineStyle','--', 'Color','k', 'LineWidth',1);
...
HealthCstR = HealthCst + (rand(size(HealthCst,1),size(HealthCst,2))-0.5)/20;
TempCst = repmat([4:2:34],size(HealthCst,1),1);
TempCst = TempCst + (rand(size(TempCst,1), size(TempCst,2))-0.5)/10;
